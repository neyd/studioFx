package com.javafxports.jfxmobile.ios.task
...
abstract class BaseIosTask extends DefaultTask {
    ...
    protected Config.Builder configure(Config.Builder builder) {
        ...
        // add extra frameworks defined in project
        project.jfxmobile.ios.frameworks.each {
            builder.addFramework(it)
        }   
        
        // add local frameworks included in project
        def iosFrameworksDir = project.file(project.jfxmobile.ios.frameworksDirectory)
        if (iosFrameworksDir.exists() && iosFrameworksDir.isDirectory()) {
            project.logger.info("Local frameworks dir: $iosFrameworksDir")
            builder.addFrameworkPath(iosFrameworksDir)
       
            project.files(iosFrameworksDir.listFiles()).each {
                if (it.exists() && it.name.endsWith('.framework')) {
                    builder.addFramework(it.name.take(it.name.lastIndexOf('.')))
                }
            }
        }
        return builder
    ...
    /**
     * Fix for IOS 10
     * Generate a new Entitlements.plist
     */
    protected Config configureDefaultEntitlementsPList(Config.Builder builder, Config config) {
        if (IOSTarget.TYPE.equals(config.targetType) ||
            (!ConsoleTarget.TYPE.equals(config.targetType) && config.os == OS.ios)) {
            builder.iosEntitlementsPList(createDefaultEntitlementsPList(config))
            return builder.build()
        } else {
            return config
        }
    ...
    /**
     * Add just the aps-environment key to allow push-notifications. Later, during codesign, the 
     * rest of the entitlements from the provisioning profile will be added
     */
    private File createDefaultEntitlementsPList(Config config) {
        File defaultEntitlementsPList = new File(config.tmpDir, "Entitlements.plist")
        defaultEntitlementsPList.withWriter { out ->
            out.writeLine("<?xml version=\"1.0\" encoding=\"UTF-8\"?>")
            out.writeLine("<!DOCTYPE plist PUBLIC \"-//Apple//DTD PLIST 1.0//EN\" \"http://www.apple.com/DTDs/PropertyList-1.0.dtd\">")
            out.writeLine("<plist version=\"1.0\">")
            out.writeLine("<dict>")
            String apsEnvironment = project.jfxmobile.ios.apsEnvironment
            if (apsEnvironment == null) {
                project.logger.debug("No iOS Aps-Environment included")
            } else if ('development' == apsEnvironment || 'production' == apsEnvironment) {
                project.logger.debug("Using iOS Aps-Environment: $apsEnvironment")
                out.writeLine("\t<key>aps-environment</key>")
                out.writeLine("\t<string>$apsEnvironment</string>")
            } else {
                throw new GradleException("Invalid iOS Aps-Environment specified: ${apsEnvironment}")
            }
            out.writeLine("</dict>")
            out.writeLine("</plist>")
        }

        return defaultEntitlementsPList
    }
