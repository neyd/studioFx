...
class IosDevice extends BaseIosTask {
    ...
    void iosDevice() {
        ...
        if (config.iosEntitlementsPList == null && project.jfxmobile.ios.apsEnvironment != null) {
            config = configureDefaultEntitlementsPList(builder, config)
        }
        ...
        project.logger.info("ios device frameworks ${config.frameworks}")
