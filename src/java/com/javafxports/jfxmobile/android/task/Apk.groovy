package com.javafxports.jfxmobile.android.task
...
class Apk extends DefaultTask {
    ...
    void apk() {
        ...
        // add jni from all jar dependencies, except for android platform's android.jar
        project.configurations.androidRuntime.filter {
            it.name.endsWith('.jar') && !it.name.endsWith('android.jar')
        }.each() {
            project.logger.info("apk: scanning ${it} for native libraries")
            project.zipTree(it).filter {
                it.name.contains('lib/armeabi-v7a')
            }.each() { file ->
                if (file.isDirectory()) {
                    project.logger.info("apk: adding ${file} to packager.addNativeLibraries()")
                    packager.addNativeLibraries(file, null)
                }
            }
        }
